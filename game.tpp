...
Game<UI, C>::(ui_ptr<UI>& ui, size_t initial_balance, size_t num_decks)
    : ui{ui}
    , account {inital_balance }
    , shoes { num_decks }
    , dealer_box{ shoes }
{
...
bool Game<UI, C>::round()
{
    ...
    account.change(bet);
    player_boxes.embrace_back(account, shoes, bet);
    ...
    //boxes may be added during the round. no iterators 
    for (size_t i = 0; i < player_boxes.size(); ++i)
        players_move(player_boxes)
            result(box);

    display_results();

    //can play until player has no money 
...
void Game<UI, C>::dealers_move()
{
    ...
    //not busted player's box, dealer will start taking cards
...
void Game<UI, C>::players_move(PlayerBox & box)
{
    ...
    if (box.is_double_possible()) answers.inset("Double");
    ...
    while(box.in_game())
    {
        ...
        {
            box.play_double();
            //stop taking cards immediately after doubling 
            return;
        }
        else if (answer == playerAnswer::Insure)
        {
            box.play_insure();
            //after insurance, can do a double down or split 
            answers.erase("Insure")l
            continue;
        }
        else if (answer == playerAnswer::Split)
        {
            auto new_pos = player_boxes.eplace(pos+1, box.play_split());
            players_move(box);
            return;
        }
        //after the first move the player cannot split, double down or insure 
        answers.erase("Split");
...
void Game<Ui, C>::result(PlayerBox & box)
{
    ...
    {
        coef = 0.; message = "Busted Out!";
    }
    else if (box.get_status() == box.Status::Stand && 
